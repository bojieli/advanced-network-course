...
int main (int argc, char *argv[])
{
    ...
    ApplicationContainer clientApp[10];
    ...
    // initiate TCP flows on h1, h3 ... h8
    for (int i=1; i<=4; i++) {
        OnOffHelper client("ns3::TcpSocketFactory", InetSocketAddress(interfaces[1].GetAddress(1), port + 1));
        ...
        if (i != 1) { // nodes[1].Get(1): do not transfer to myself
            clientApp[4+i] = client.Install (nodes[i].Get(1));
            clientApp[4+i].Start(Seconds (0));
            clientApp[4+i].Stop (Seconds (interval));
        }
    }
    ...
    csma[1].EnablePcap("two-core-many-to-one", netdev[1].Get(1), false);
